...
Class('Security', {
  // where possible, these attributes should be named consistently with the terms in the Legalese spreadsheets.
  has: { name:      {is:'ro',required:true, },
		 corp:      {is:'ro',required:true, },           // the corporation i belong to
		 voting:    {is:'ro',required:false,init:function(){return 0}},
		 redeemableByCompany:{is:'ro',required:true, },  // false || function -- when is it redeemable?
		 redeemableByHolder: {is:'ro',required:true, },  // false || function
		 liquidationPreference: {is:'rw'},               // N
		 acquisitionPreference: {is:'rw'},               // if acquired before conversion
		 participation: {is:'rw'},                       // at liquidation, in the assets of the company
		 interestRate:  {is:'ro',required:true,},        // between 0 and 1
		 maturityDate:  {is:'ro',required:true,},        // javascript Date type? or YYYY-MM-DD
		 atMaturity:    {is:'rw',required:false},        // returns a new Security
		 convertsTo:    {is:'rw',required:true},         // "Next Round Security' || "Ordinary Shares" || Security
		 convCondition: {is:'rw',required:false},        // function representing the qualifying conditions for conversion
		 convDiscount:  {is:'rw',required:false},        // N || function
		 convCap:       {is:'rw',required:false},        // N
		 information:   {is:'rw',required:false},        // information rights
		 majorInvestor: {is:'rw',required:false},        // information rights
		 MFN:           {is:'rw',required:false},        // most favoured nation clause.
	   },
  method: {
	characterize: function()      { return }, // based on US GAAP, this is a Blah. based on SG FRS, this is a bleh. etc.
	eventHandler: function(event) {
	  switch (event.name) {
	  case "financing":
		if (this.convCondition(event)) {
		  this.convert();
		}
	  default:
		return;
	...
	convert: function() {
	  // edit the corp's cap table. log to the trail of paperwork to be issued. change the holder's holding of this security to some other holding.
	},
  },
});

var KISS = new Security({
  convertsTo: "nextRoundSecurity",
  convCondition: function(event){
	if (event.totalFundsRaised > 250000 &&
		event.nextRoundSecurity
	   ) {
	  return true;
	}
  }
...
Class('Corp', {
  ...
  has: { name       : {is:'rw',required:true,  },
		 ...
		 rules      : {is:'rw',init: function(){
		   // node : [ node1, node2, [ node3, node4] ] = parent node requires node1 && node2 && (node3 || node4)
		   // node ::= string | function
		   //                   function will determine whether the condition is met or not. if it is met, return null; if it is not met, return a string describing requirement.
		   return {
			 newRound: [ [ 'securityExists', 'newSecurity' ],
						 'members161' ],
			 securityExists: [ function(counterfactual) { return (this.getSecurities()[counterfactual.params.securityName] ? null : "must create security") } ],
			 newSecurity: [ 'membersSpecialResolution' ],
			 members161:  [ 'existingAuthorization', 'membersNotice'],
			 membersNotice: [ 'directorsResolution' ],
			 directorsResolution: [ [ function(counterfactual) { return "director's resolution" } ],
		 }},
	   },
  methods : {
	display: function (params) {
	...
	rMembers161: function(counterfactual, prerequisites) {
